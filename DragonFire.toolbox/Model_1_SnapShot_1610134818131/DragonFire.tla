...
thread1Enter: while TRUE do
    A:
        if c < 2 then
            ...
            critical_section1: skip;
        end if;
end while;
end process;
end algorithm *)
\* BEGIN TRANSLATION (chksum(pcal) = "274ad8cf" /\ chksum(tla) = "28819a69")
...
if3 == /\ pc[0] = "if3"
       /\ IF (fireball > 0)
             THEN /\ fireball' = fireball - 1
                  /\ pc' = [pc EXCEPT ![0] = "critical_section0"]
             ELSE /\ pc' = [pc EXCEPT ![0] = "c1"]
                  /\ UNCHANGED fireball
       /\ UNCHANGED << firebrathing, c, temp >>

critical_section0 == /\ pc[0] = "critical_section0"
                     /\ TRUE
                     /\ pc' = [pc EXCEPT ![0] = "c1"]
                     /\ UNCHANGED << fireball, firebrathing, c, temp >>
...
thread0 == thread0Enter \/ MonitorEnter \/ incinerate_enemies \/ if1
              \/ blast_enemies \/ if2 \/ if3 \/ critical_section0 \/ c1
...
A == /\ pc[1] = "A"
     /\ IF c < 2
           ...
           ELSE /\ pc' = [pc EXCEPT ![1] = "critical_section1"]
     /\ UNCHANGED << fireball, firebrathing, c, temp >>
...
critical_section1 == /\ pc[1] = "critical_section1"
                     /\ TRUE
                     /\ pc' = [pc EXCEPT ![1] = "thread1Enter"]
                     /\ UNCHANGED << fireball, firebrathing, c, temp >>

thread1 == thread1Enter \/ A \/ release \/ critical_section1
...
\* END TRANSLATION 
NotDoubleCriticalSection == pc[0] /= "critical_section0" /\ pc[1] /= "critical_section1"
=============================================================================
\* Modification History
\* Last modified Fri Jan 08 22:39:46 MSK 2021 by ruslan
